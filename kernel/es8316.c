...
struct es8316_priv {
	...
	bool jd_inverted;
	unsigned long last_irq_jiffies; /* IRQ 软件 debounce：限制最小处理间隔 */
};
...
static irqreturn_t es8316_irq(int irq, void *data)
{
	...
	/* 软件 debounce：快速拔插时限制最小处理间隔为 50ms，
	 * 避免在 msleep(20) + dapm_sync 期间累积的 IRQ 导致
	 * codec I2C 从接口因竞争挂死。
	 */
	if (es8316->last_irq_jiffies &&
	    time_before(jiffies, es8316->last_irq_jiffies + msecs_to_jiffies(50)))
		return IRQ_HANDLED;
	es8316->last_irq_jiffies = jiffies;

	mutex_lock(&es8316->lock);

	/* regmap_read 加错误检查：I2C 挂死时避免永久阻塞 mutex */
	ret = regmap_read(es8316->regmap, ES8316_GPIO_FLAG, &flags);
	if (ret) {
		dev_warn_ratelimited(comp->dev, "GPIO_FLAG read failed: %d\n", ret);
		goto out;
	}
	if (flags == 0x00)
	...
	if (flags & ES8316_GPIO_FLAG_HP_NOT_INSERTED) {
		 ...
		 * 关键修复：将 micbias 使能 + msleep(20) + 二次 regmap_read
		 * 移出 es8316->lock，避免持有 mutex 期间 sleep 和 I2C 访问，
		 * 这会与 DAPM/PCM trigger 的 regmap 访问死锁，并导致
		 * ES8316 I2C 从接口在快速拔插时挂死。
		 */
		...
		mutex_lock(&es8316->lock);
		if (ret) {
			dev_warn_ratelimited(comp->dev, "GPIO_FLAG re-read failed: %d\n", ret);
			goto out;
		}
		if (es8316->jd_inverted)
			flags ^= ES8316_GPIO_FLAG_HP_NOT_INSERTED;
