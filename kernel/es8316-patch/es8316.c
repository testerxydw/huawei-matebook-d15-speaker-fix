...
static irqreturn_t es8316_irq(int irq, void *data)
{
	...
	if (flags & ES8316_GPIO_FLAG_HP_NOT_INSERTED) {
		 ...
		 *
		 * FIX: do not disable the micbias/bias power domains here.
		 * Disabling them powers the codec down into deep shutdown
		 * (the I2C interface stops responding), which also kills the
		 * JD detect circuit and freezes the interrupt pin until the
		 * next reboot, breaking all further plug/unplug detection.
		 */
				      ...
				      struct snd_soc_jack *jack)
{
	...
	/*
	 * FIX: keep the JD related power domains always on while jack
	 * detection is enabled. Otherwise the codec can be powered down
	 * into deep shutdown (I2C stops responding) after jack removal,
	 * which kills the JD detect circuit and freezes the interrupt
	 * pin until reboot.
	 */
...
static int es8316_i2c_probe(struct i2c_client *i2c_client)
{
	...
	if (es8316->irq > 0) {
		/*
		 * FIX: use both edges so that unplugging (falling edge) also
		 * triggers the interrupt, matching the ACPI GpioInt
		 * (Edge, ActiveBoth) resource of the ESSX8336 device.
		 */
		ret = devm_request_threaded_irq(dev, es8316->irq, NULL, es8316_irq,
						IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT | IRQF_NO_AUTOEN,
						"es8316", es8316);
		if (ret) {
